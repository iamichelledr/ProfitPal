...
export default function CalculatorPage() {
  ...
  const calculate = () => {
    ...
    const breakEvenQuantity = Math.ceil(totalBatchCost / suggestedSRP);
  ...
  const costInputs = [
    { field: 'materialsCost', label: 'Materials Cost', icon: Package, placeholder: '0.00', description: 'Raw materials and ingredients' },
    { field: 'laborCost', label: 'Labor Cost', icon: Users, placeholder: '0.00', description: 'Your time or wages paid' },
    { field: 'packagingCost', label: 'Packaging Cost', icon: Package, placeholder: '0.00', description: 'Boxes, labels, bags' },
    { field: 'utilitiesCost', label: 'Utilities Cost', icon: Zap, placeholder: '0.00', description: 'Electricity, gas, water' },
    { field: 'platformFees', label: 'Platform/Delivery Fees', icon: DollarSign, placeholder: '0.00', description: 'Online marketplace fees' },
    { field: 'otherCosts', label: 'Other Costs', icon: Info, placeholder: '0.00', description: 'Rent, equipment, misc.' },
  ];
              ...
              <CardContent className="space-y-6">
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                  {costInputs.map((input) => (
                    <div key={input.field}>
                      <Label htmlFor={input.field} className="text-pp-dark font-medium flex items-center gap-2">
                        <input.icon className="w-4 h-4 text-pp-blue" />
                        {input.label}
                      </Label>
                      <div className="relative mt-1">
                        <span className="absolute left-3 top-1/2 -translate-y-1/2 text-pp-slate">₱</span>
                        <Input
                          id={input.field}
                          type="number"
                          min="0"
              ...
              {!showResults ? (
                ...
                <>
                  ...
                  {formData.userTargetPrice && result.userPriceProfit > 0 && (
                    <Card className={`shadow-card ${result.profitDifference >= 0 ? 'border-pp-green/20' : 'border-amber-400/50'}`}>
                      <CardHeader className={result.profitDifference >= 0 ? 'bg-pp-green/5' : 'bg-amber-50'}>
                        <CardTitle className={`flex items-center gap-2 ${result.profitDifference >= 0 ? 'text-pp-green' : 'text-amber-600'}`}>
                          {result.profitDifference >= 0 ? <CheckCircle2 className="w-5 h-5" /> : <AlertTriangle className="w-5 h-5" />}
                          Price Comparison
                        </CardTitle>
                      </CardHeader>
                      <CardContent className="pt-4">
                        <div className="grid sm:grid-cols-2 gap-4">
                          <div>
                            ...
                            <p className={`text-lg font-bold ${result.userPriceProfit >= 0 ? 'text-pp-green' : 'text-red-500'}`}>
                              {formatCurrency(result.userPriceProfit)}
                            </p>
                          </div>
                          <div>
                            <p className="text-sm text-pp-slate mb-1">Difference</p>
                            <p className={`text-lg font-bold ${result.profitDifference >= 0 ? 'text-pp-green' : 'text-amber-600'}`}>
                              {result.profitDifference >= 0 ? '+' : ''}{formatCurrency(result.profitDifference)}
                            </p>
                          </div>
                        </div>
            ...
            {[
              {
                ...
                description: 'Don\'t forget hidden costs like utilities, packaging, and platform fees.',
              },
              {
                title: 'Know Your Market',
                description: 'Research competitor prices while ensuring you cover all costs.',
              },
              {
                title: 'Test Different Margins',
                description: 'Try different profit margins to find the sweet spot for your market.',
              },
            ].map((tip, index) => (
